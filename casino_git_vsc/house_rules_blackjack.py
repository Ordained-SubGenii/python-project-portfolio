...
class Game():
    ...
    def rules(self) -> None:
        ...
        if request_rules in {"y","Y"}:
            ...
            At the onset of the round, both dealer and player are delt 2 cards, but, only the top 
            card of the dealer is visible. If the sum of the 2 player cards is 21, a "black jack" is achieved and the round ends. 
            ...
            While only seeing the top card of the dealer, the player decides if they want to try to increase their total with addition of 
            ...
            the dealer must subsequently hit so long as the dealer has less than or equal to 16 after 
...
class BlackJack(Game):
    ...
    def deal(self) -> tuple[list,int,list,int]: 
        ...
        dealer_hand.extendstack(deck.rmcard_frmstack(1))
    ...
    def player_hand_win(self):
        return True if dealer_hand.stacksum < player_hand.stacksum <= 21 or dealer_hand.stacksum > 21 else False
